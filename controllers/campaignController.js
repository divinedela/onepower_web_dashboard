...
const addCampaign = async (req, res) => {
    ...
    try {
        ...
        if (loginData && loginData.isAdmin === 0) {
            ...
            req.flash('error', 'You do not have permission to add campaign. As a demo admin, you can only view the content.');
        ...
        const organizer_image = req.files['organizer_image'] ? req.files['organizer_image'][0].filename : null;
        ...
        const newCampaign = await new campaignModel({
            name, categoryId, starting_date, ending_date, campaign_amount: amount, organizer_name, description, image, organizer_image, gallery
        }).save();

        if (!newCampaign) {
            req.flash('error', 'Campaign could not be added. Please make sure all required fields are filled.');
            return res.redirect(process.env.BASE_URL + "add-campaign");
        }
...
const loadCampaign = async (req, res) => {
    ...
    try {
        ...
        const campaign = await campaignModel.find({ isUser: false }).populate("categoryId").sort({ createdAt: -1 });

        const updatedCampaignData = await combineCampaignAndDonation(campaign);

        const loginData = await adminLoginModel.find();

            return res.render("campaign", { campaign: updatedCampaignData, loginData, IMAGE_URL: process.env.IMAGE_URL });
        });
    } catch (error) {
        console.log(error.message);
...
const loadUserCampaign = async (req, res) => {
    ...
    try {
        ...
        await verifyAdminAccess(req, res, async () => {
            ...
            const campaign = await campaignModel.find({ isUser: true }).populate("categoryId userId").sort({ createdAt: -1 });

            const updatedCampaignData = await combineCampaignAndDonation(campaign);

            const loginData = await adminLoginModel.find();

            return res.render("userCampaign", { campaign: updatedCampaignData, loginData, IMAGE_URL: process.env.IMAGE_URL });
        });
    } catch (error) {
        console.log(error.message);
...
const loadCampaignInfo = async (req, res) => {
    ...
    try {
        ...
        const donor = await donationModel.find({ campaignId: id }).populate("userId campaignId");

        const updatedCampaignData = await combineCampaignAndDonation(campaign);

        return res.render("campaignInfo", { campaign: updatedCampaignData, donor, IMAGE_URL: process.env.IMAGE_URL });
...
const loadEditCampaign = async (req, res) => {
    ...
    try {
        ...
        return res.render("editCampaign", { campaign, categoryData, IMAGE_URL: process.env.IMAGE_URL });
...
const editCampaign = async (req, res) => {
    ...
    try {
        ...
        if (req.body.ending_date < req.body.starting_date) {
            ...
            if (req.files && req.files['organizer_image'] && req.files['organizer_image'][0]) {
                // Delete old image
                deleteImage(req.files['organizer_image'][0].filename);
            }
            req.flash('error', 'Ending date must be after starting date.');
        ...
        const updatedCampaign = await campaignModel.findOneAndUpdate(
            ...
            {
                $set: {
                    name, categoryId, starting_date, ending_date, campaign_amount: amount, organizer_name, organizer_image, image, description
                }
            },
            { new: true }
        );
        ...
        if (!updatedCampaign) {
            req.flash('error', 'Campaign could not be updated. Please make sure all required fields are filled.');
            return res.redirect(process.env.BASE_URL + "edit-campaign?id=" + id);
        }
...
const approveCampaign = async (req, res) => {
    ...
    try {
        ...
        const updatedCampaign = await campaignModel.findByIdAndUpdate(
            id,
            { $set: { isApproved: campaign.isApproved === true ? false : true, status: campaign.isApproved === true ? "UnPublish" : "Publish" } },
            { new: true }
        );
...
const addGalleryImage = async (req, res) => {
    ...
    try {
        ...
        await campaignModel.updateOne({ _id: id }, { $set: { gallery: gallery.concat(galleryImage) } });
...
const editGalleryImage = async (req, res) => {
    ...
    try {
        ...
        await campaignModel.findOneAndUpdate({ _id: id, 'gallery': oldImage }, { $set: { 'gallery.$': galleryImage } }, { new: true });
...
const deleteGalleryImage = async (req, res) => {
    ...
    try {
        ...
        await campaignModel.findByIdAndUpdate({ _id: id }, { $pull: { gallery: { $in: [gallery] } } }, { new: true });
...
const loadDonation = async (req, res) => {
    ...
    try {
        ...
        return res.render("donation", { campaign: updatedCampaignData, donor, IMAGE_URL: process.env.IMAGE_URL });
