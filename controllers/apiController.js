...
const signUp = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            const { firstname, lastname, email, country_code, phone_number, password } = req.body;
            ...
            // Save user
            const newUser = new userModel({ firstname, lastname, email, country_code, phone_number, password: hashedPassword });
...
const verifyOTP = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            const updatedUser = await userModel.findOneAndUpdate({ email }, { $set: { isVerified: true, status: 'Active' } });
...
const forgotPasswordOtpVerification = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            return res.json({ success: 1, message: "OTP verified successfully", error: 0 });
    ...
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            if (!otpRecord.isVerified) {
                return res.status(400).json({ data: { success: 0, message: "Please verify your OTP", error: 1 } });
            }
...
const getUserDetails = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            const user = await userModel.findOne({ _id: userId }, { password: 0, isVerified: 0, registeredBy: 0 });
...
const editUserProfile = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            // update user details
            const updatedUser = await userModel.findOneAndUpdate({ _id: userId }, { $set: { firstname, lastname, country_code, phone_number, image } }, { new: true });
...
const deleteAccountUser = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            // delete donation
            await donationModel.updateMany({ userId: userId }, { $set: { userId: null } });

            // delete campaign
            await campaignModel.updateMany({ userId: userId }, { $set: { userId: null } });
...
const getAllIntro = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            if (!intro.length) return res.json({ data: { success: 0, message: "Intro Not Found", intro: intro, error: 1 } });
...
const getAllBanner = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            // fetch banner
            const banner = await bannerModel.find({ status: "Publish" }, { status: 0 }).populate({
                path: "campaignId",
            ...
            if (!banner.length) return res.json({ data: { success: 0, message: "Banner Not Found", banner: banner, error: 1 } });

            // // Update banner data
            const updatedBannerData = await Promise.all(
                banner.map(async (item) => {
                    const campaignData = await campaignModel.findOne({ _id: item.campaignId }, { status: 0, isApproved: 0, userId: 0, isUser: 0 }).populate("categoryId", "_id image name");
                    const updatedCampaignData = await combineCampaignAndDonation(campaignData);
                    return {
                        ...item.toObject(),
                        campaignId: updatedCampaignData
                    };
                })
            );

            return res.json({ data: { success: 1, message: "Banner Found", banner: updatedBannerData, error: 0 } });
        });
    } catch (error) {
...
const comingToEndCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            return res.json({ data: { success: 1, message: "coming to end campaign Found", campaigns: updatedCampaignData, error: 0 } });
        });
    } catch (error) {
...
const getAllCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            // fetch campaign
            const campaignData = await campaignModel.find(filter, { status: 0, isApproved: 0, userId: 0, isUser: 0 }).populate("categoryId", "_id image name");

            if (!campaignData.length) return res.json({ data: { success: 0, message: "Campaign Not Found", campaigns: [], error: 1 } });

            // combine campaign and donation
            const updatedCampaignData = await combineCampaignAndDonation(campaignData);

            return res.json({ data: { success: 1, message: "Campaign Found", campaigns: updatedCampaignData, error: 0 } });
        });
    } catch (error) {
...
const addCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            const fields = { image, name, categoryId, starting_date, ending_date, campaign_amount, organizer_image, organizer_name, description };
            const missingFields = [];
            ...
            if (missingFields.length > 0) {
                return res.json({ data: { success: 0, message: `Missing required fields: ${missingFields.join(', ')}`, error: 1 } });
            }

            const category = await categoryModel.findOne({ _id: categoryId });

            if (!category) return res.json({ data: { success: 0, message: "Category Not Found", error: 1 } });
            ...
            if (!startingDateString.isValid() || !endingDateString.isValid()) return res.json({ data: { success: 0, message: "Invalid Date, please enter a valid date", error: 1 } });
            ...
            if (startingDate < currentDate || endingDate < currentDate)
                return res.json({ data: { success: 0, message: "Please choose a date that is today or in the future. Past dates cannot be selected for campaign.", error: 1 } });

            if (startingDate > endingDate)
                return res.json({ data: { success: 0, message: "Please ensure that the starting date is before the ending date.", error: 1 } });

            // Save the campaign
            const savedCampaign = await new campaignModel({
                image, name, categoryId, starting_date, ending_date, campaign_amount, organizer_image, organizer_name,
                description, gallery, userId, isUser: true, status: "UnPublish"
            }).save();

            if (!savedCampaign) return res.json({ data: { success: 0, message: "Failed to save campaign", error: 1 } });

            // Fetch user
            const findUser = await userModel.findOne({ _id: userId });

            // Send notification to admin
            await sendAdminNotification("New Campaign Added", `A new campaign ${name} has been added by ${findUser.firstname} ${findUser.lastname}`);

            return res.json({ data: { success: 1, message: "Campaign added successfully", error: 0 } });
        });
    } catch (error) {
...
const getAllUserCampaign = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            const campaignData = await campaignModel.find({ userId, isUser: true }, { status: 0, userId: 0, isUser: 0 })
                .populate("categoryId", "_id image name").sort({ createdAt: -1 });

            if (!campaignData.length) return res.json({ data: { success: 0, message: "Campaign Not Found", campaigns: campaignData, error: 1 } });
...
const deleteCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            const campaignData = await campaignModel.findOne({ _id: campaignId, userId, isUser: true });

            if (!campaignData) return res.json({ data: { success: 0, message: "This campaign is not belong to you", error: 1 } });

            const updatedCampaignData = await combineCampaignAndDonation(campaignData);

            if (["Running", "Ended"].includes(updatedCampaignData.campaign_status)) {
                return res.json({ data: { success: 0, message: `You can't delete the ${updatedCampaignData.name} campaign because it's already ${updatedCampaignData.campaign_status}`, error: 1 } });
            }

            const deletedCampaign = await campaignModel.deleteOne({ _id: campaignId, userId, isUser: true });

            if (deletedCampaign.deletedCount === 0) {
                return res.json({ data: { success: 0, message: "Failed to delete campaign", error: 1 } });
            }

            return res.json({ data: { success: 1, message: "Campaign deleted successfully", error: 0 } });
        });
    } catch (error) {
    ...
    try {
        await verifyAccess(req, res, async () => {
            ...
            if (payment_status === "Failed") {
                return res.json({ data: { success: 0, message: "Payment Failed so donation not added", error: 1 } });
            }

            // Validate the donation amount 
            if (!amount || isNaN(amount) || amount <= 0) {
                return res.json({ data: { success: 0, message: "Invalid donation amount", error: 0 } });
            }
            ...
            if (!campaignData) {
                return res.json({ data: { success: 0, message: "Campaign Not Found", error: 1 } });
            }

            // Save the donation 
            const savedDonation = await new donationModel({ userId, campaignId, amount: amount, date: currentDate, payment_method, transaction_id, payment_status }).save();

            if (!savedDonation) return res.json({ data: { success: 0, message: "Failed to donate amount. Please try again later.", error: 1 } });

            return res.json({ data: { success: 1, message: "Donation added successfully", error: 0 } });
...
const getAllDonateHistory = async (req, res) => {
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            const donateHistory = await donationModel.find({ userId }).populate("campaignId userId", "_id name image firstname lastname");

            if (!donateHistory.length) {
                return res.json({ data: { success: 0, message: "No donate history found", donateHistory, error: 1 } });
            }

            return res.json({ data: { success: 1, message: "Donate history found", donateHistory, error: 0 } });
        });
    } catch (error) {
...
const addFavouriteCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            // Validate input
            if (!campaignId) return res.status(400).json({ data: { success: 0, message: "Campaign is required.", error: 1 } });

            // Check if the campaign is already a favourite for the user
            const existingFavourite = await favouriteCampaignModel.findOne({ userId, campaignId });
            if (existingFavourite) {
                return res.status(409).json({ data: { success: 0, message: "This campaign is already in your favourites.", error: 1 } });
            }

            // Save the favourite campaign
            const savedFavouriteCampaign = await new favouriteCampaignModel({ userId, campaignId }).save();

            if (!savedFavouriteCampaign) {
                return res.json({ data: { success: 0, message: "Campaign not added to favourites successfully", error: 1 } });
            }

            // Respond with success
            return res.status(201).json({ data: { success: 1, message: "Campaign added to favourites successfully.", error: 0 } });
        });
    } catch (error) {
    ...
    try {
        ...
        await verifyAccess(req, res, async () => {
            ...
            if (!favouriteCampaigns.length) {
                return res.status(404).json({ data: { success: 0, message: "No favourite campaigns found for this user", campaigns: favouriteCampaigns, error: 1 } });
            }

            const favouriteCampaignIds = favouriteCampaigns.map(fc => fc.campaignId);

            // fetch campaign
            const campaignData = await campaignModel.find({ _id: { $in: favouriteCampaignIds }, status: "Publish" }, { status: 0, isApproved: 0, userId: 0, isUser: 0 })
                .populate("categoryId", "_id image name");

            // combine campaign and donation
            const updatedCampaignData = await combineCampaignAndDonation(campaignData);

            return res.status(200).json({ data: { success: 1, message: "Favourite campaigns retrieved successfully", campaigns: updatedCampaignData, error: 0 } });
        });
    } catch (error) {
...
const deleteFavouriteCampaign = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            const deletedFavouriteCampaign = await favouriteCampaignModel.deleteOne({ userId, campaignId });

            // Check if favourite campaign was deleted
            if (deletedFavouriteCampaign.deletedCount === 0) {
                return res.status(404).json({ data: { success: 0, message: "Favourite campaign not found", error: 1 } });
            }

            return res.status(200).json({ data: { success: 1, message: "Favourite campaign deleted successfully", error: 0 } });
        }); S
    } catch (error) {
...
const getAllNotification = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            const notification = await notificationModel.find({ recipient: "User", createdAt: { $gte: userLoginDate } }, { recipient: 0, recipient_user: 0, is_read: 0 }).sort({ createdAt: -1 });

            if (!notification.length) {
                return res.json({ data: { success: 0, message: "No notification found", notification, error: 1 } });
            }

            return res.json({ data: { success: 1, message: "Notification found", notification, error: 0 } });
        });
    } catch (error) {
    ...
    try {
        await verifyAccess(req, res, async () => {
            ...
            if (!page) {
                ...
                return res.json({ data: { success: 0, message: "Page Not Found", page, error: 1 } });
            }
            else {

                return res.json({ data: { success: 1, message: "Page Found", page, error: 0 } });
            }
...
const getCurrency = async (req, res) => {
    try {
        await verifyAccess(req, res, async () => {
            ...
            if (!currencyTimezone) return res.json({ data: { success: 0, message: "Currency Not Found", currency: currencyTimezone, error: 1 } });

            return res.json({ data: { success: 1, message: "Currency Found", currency: currencyTimezone, error: 0 } });
        });
    } catch (error) {
